...
use Alynt\CertificateGenerator\AdminUi\Tabs\Alynt_Certificate_Generator_Tab_Diagnostics;
...
/**
 * Registers and renders the plugin settings screens.
 */
class Alynt_Certificate_Generator_Settings {
	...
	public function render_page(): void {
		...
		if ( 'fonts' === $active_tab ) {
			$fonts_page = new Alynt_Certificate_Generator_Fonts_Settings_Page();
			$fonts_page->render();
			...
			$diagnostics_page = new Alynt_Certificate_Generator_Diagnostics_Page();
	...
	private function get_active_tab( bool $from_post = false ): string {
		if ( $from_post && isset( $_POST['alynt_active_tab'], $_POST['_wpnonce'] ) ) {
			$nonce = sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) );
			if ( wp_verify_nonce( $nonce, 'alynt_certificate_generator_settings_group-options' ) ) {
				return sanitize_key( wp_unslash( $_POST['alynt_active_tab'] ) );
			}
		}

		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- The tab query arg only controls read-only admin navigation and is sanitized before use.
		if ( isset( $_GET['tab'] ) ) {
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- The tab query arg only controls read-only admin navigation and is sanitized before use.
	...
	private function get_tabs(): array {
		return array(
			...
			new Alynt_Certificate_Generator_Tab_Diagnostics(),
		);
	}
